# --
# Kernel/Modules/AgentFileManager4.pm - a file download manager
...
package Kernel::Modules::AgentFileManager4;
...
sub new {
    ...
    # config options
    $Self->{DataDir}  = $Self->{ConfigObject}->Get('FileManager4::Root') . "/";
    $Self->{TrashDir} = $Self->{ConfigObject}->Get('FileManager4::Trash')
        || $Self->{DataDir} . '/Trash/';
    $Self->{TrashDir} =~ s{<OTRS_FileManager4::Root>}{$Self->{DataDir}}smx;

    $Self->{ReadAccessMap} = $Self->{ConfigObject}->Get('FileManager4::ReadAccessMap')
        || { '/' => 'users' };
    $Self->{DeleteAccessMap} = $Self->{ConfigObject}->Get('FileManager4::DeleteAccessMap')
        || { '/' => 'users' };
    $Self->{CreateAccessMap} = $Self->{ConfigObject}->Get('FileManager4::CreateAccessMap')
        || { '/' => 'users' };
...
sub Run {
    ...
    for my $File (@List) {
        ...
        if ( $Self->{ConfigObject}->Get("FileManager4::ListType") ) {
    ...
    $Output .= $Self->{LayoutObject}->Output(
        TemplateFile => 'AgentFileManager4',
        Data         => {
            %Param,
            Location => $Self->{Location},
        },
