# --
# Kernel/Modules/CustomerDownload4.pm - frontend module
...
package Kernel::Modules::CustomerDownload4;
...
sub new {
    ...
    # config options
    $Self->{DataDir}  = $Self->{ConfigObject}->Get('CustomerDownload4::Root') . "/" . $Self->{UserID} . "/";
    $Self->{TrashDir} = $Self->{ConfigObject}->Get('CustomerDownload4::Trash')
        || $Self->{DataDir} . '/Trash/';
    $Self->{TrashDir} =~ s{<OTRS_CustomerDownload4::Root>}{$Self->{DataDir}}smx;

    $Self->{ReadAccessMap} = $Self->{ConfigObject}->Get('CustomerDownload4::ReadAccessMap')
        || { '/' => 'users' };
    $Self->{DeleteAccessMap} = $Self->{ConfigObject}->Get('CustomerDownload4::DeleteAccessMap')
        || { '/' => 'users' };
    $Self->{CreateAccessMap} = $Self->{ConfigObject}->Get('CustomerDownload4::CreateAccessMap')
        || { '/' => 'users' };
...
sub Run {
    ...
    if ( !-e $Self->{DataDir} ) {
        return $Self->{LayoutObject}->ErrorScreen(
            Message =>
                "FileManager Root directory doesn't exist: $Self->{DataDir}!\n"
                . "This means:\n"
                . " o your administrator did not created your home directory\n"
                . "   please contact your administrator to creat it.\n",
        );
    }
    ...
    for my $File (@List) {
        ...
        if ( $Self->{ConfigObject}->Get("CustomerDownload4::ListType") ) {
    ...
    # browse the tree
    my $Output = $Self->{LayoutObject}->CustomerHeader( Title => 'Downloads' );
    $Output .= $Self->{LayoutObject}->CustomerNavigationBar();
    $Output .= $Self->{LayoutObject}->Output(
        TemplateFile => 'CustomerDownload4',
        Data         => {
            %Param,
            Location => $Self->{Location},
        },
